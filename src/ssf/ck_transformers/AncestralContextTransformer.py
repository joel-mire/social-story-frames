...
class AncestralContextTransformer(Transformer):
  def __init__(self, 
               dir,
               force_rebuild,
               k,
               ...
               meta_key=ANCESTRAL_CONTEXT_META_KEY):
    self.dir = dir
  ...
  def _get_prompts(self, ancestor_summaries_list):
    prompts = []
    for ancestor_summaries in ancestor_summaries_list:
      summary_count = len(ancestor_summaries)
      if summary_count == 0:
        prompts.append(NO_OP_MSG)
        continue
      prompt = f"Below are {summary_count} summaries social media posts in an ancestral chain (parent-child relationships as you read left to right). Your task is to generate a global summary of the overall chain based on the local summaries in three sentences or less."
  ...
  def transform(self, corpus: Corpus, selector=None):
    prompts_path = f'{self.dir}/prompts/ancestralcontext{self.k}.jsonl'
    outputs_path = f'{self.dir}/outputs/ancestralcontext{self.k}.jsonl'
