...
class ImplausibleInferenceGenerationPromptBuilder:
  ...
  def _build_task_statement(self):
    return f"Your task is to generate one contextually **implausible** description of the {TAXONOMY_TASK_SINGULAR_DICT[self._dim]} in a social media conversation."
  ...
  def _build_single_output_instructions(self):
    ...
    output_instructions = (
        'Output Instructions:'
        f'\nRemember: {self._build_task_statement()}, Your output will be used to test human annotators, to see if they correctly identify your response as implausible or extremely unlikely.'
        '\nFill in the JSON list below with **exactly 1 contextually implausible/highly unlikely response** to the current text in conversational context.'
